...
function refreshGithubTrending() {
  ...
  .then(([ jsData, tsData ]) => {
    ...
    const jsUpload = firebase.database().ref('content/github/trending/javascript').set(jsData)
      .then(() => jsData)
      .catch(err => {
    ...
    const tsUpload = firebase.database().ref('content/github/trending/typescript').set(tsData)
      .then(() => tsData)
      .catch(err => {
